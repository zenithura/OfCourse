...
CORS(app, 
     resources={
         r"/api/*": {
             "origins": os.getenv('ALLOWED_ORIGINS', 'https://of-course-nine.vercel.app').split(','),
...
try:
    # Public istemci (okuma işlemleri için)
    public_supabase: Client = create_client(
        os.getenv('SUPABASE_URL'),
        os.getenv('SUPABASE_KEY')
    )
    
    # Service role istemcisi (yazma işlemleri için)
...
def admin_login():
    try:
        ...
        return jsonify({'error': 'Login işlemi başarısız'}), 500
...
def upload_file_to_storage(file, bucket_name):
    ...
    try:
        ...
        # Public URL oluştur
        public_url = f"{os.getenv('SUPABASE_URL')}/storage/v1/object/public/{bucket_name}/{unique_filename}"
        return public_url
        ...
        if search:
            ...
            for course in response_data:
                if 'image' in course and not course['image'].startswith('http'):
                    course['image'] = f"{os.getenv('SUPABASE_URL')}/storage/v1/object/public/images/{course['image'].split('/')[-1]}"
                if 'downloadUrl' in course and not course['downloadUrl'].startswith('http'):
                    course['downloadUrl'] = f"{os.getenv('SUPABASE_URL')}/storage/v1/object/public/downloads/{course['downloadUrl'].split('/')[-1]}"
...
def download_course_file(course_id):
    try:
        ...
        if not download_url.startswith(os.getenv('SUPABASE_URL')):
            # Tam URL'yi oluştur
            bucket_name = 'downloads'
            file_path = download_url.split('/')[-1]  # URL'den dosya adını al
            download_url = f"{os.getenv('SUPABASE_URL')}/storage/v1/object/public/{bucket_name}/{file_path}"
