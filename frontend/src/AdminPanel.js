...
export const AdminPanel = () => {
  ...
  const handleLogin = useCallback(async (e) => {
    e.preventDefault();
    try {
      await axios.post('http://localhost:5000/api/admin/login', loginData, { withCredentials: true });
  ...
  const handleDelete = useCallback(async (courseId) => {
    if (window.confirm('Bu kursu silmek istediğinizden emin misiniz?')) {
      try {
        await axios.delete(`http://localhost:5000/api/admin/courses/${courseId}`, { withCredentials: true });
        fetchCourses();
      } catch (error) {
  ...
  const handleRemoveDiscounts = async () => {
    ...
    if (window.confirm('Seçili kursların indirimlerini kaldırmak istediğinizden emin misiniz?')) {
      try {
        await axios.post('http://localhost:5000/api/admin/remove-discounts', 
