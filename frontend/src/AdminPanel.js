...
export const AdminPanel = () => {
  ...
  const checkAuth = useCallback(async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/api/admin/check-auth`, { withCredentials: true });
  ...
  const handleLogin = useCallback(async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${API_BASE_URL}/api/admin/login`, loginData, { withCredentials: true });
  ...
  const handleDelete = useCallback(async (courseId) => {
    if (window.confirm('Bu kursu silmek istediğinizden emin misiniz?')) {
      try {
        await axios.delete(`${API_BASE_URL}/api/admin/courses/${courseId}`, { withCredentials: true });
        fetchCourses();
      } catch (error) {
  ...
  const handleRemoveDiscounts = async () => {
    ...
    if (window.confirm('Seçili kursların indirimlerini kaldırmak istediğinizden emin misiniz?')) {
      try {
        await axios.post(`${API_BASE_URL}/api/admin/remove-discounts`, 
