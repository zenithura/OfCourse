...
export const CreateCourse = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        const authResponse = await axios.get('http://localhost:5000/api/admin/check-auth', { withCredentials: true });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.message) {
        ...
        throw new Error(response.data.error || 'Oluşturma başarısız');
      }
    } catch (error) {
      console.error('Oluşturma hatası:', error.response || error);
      alert(error.response?.data?.error || error.message || 'Kurs oluşturulamadı!');
    }
  };
